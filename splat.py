...
@slash.slash(name="ping", description="Test command", guild_ids=guild_ids)
...
@slash.slash(name="help", description="Detailed descriptions of Lil Judd's commands", guild_ids=guild_ids)
async def _help(ctx):
    ...
    embed.add_field(name="/callouts", value="Sends the image with annotated callouts corresponding to the given map. The map either has to match the name of the map or its short code. If the optional argument rm is set to True, the rainmaker path for the given map will be sent instead", inline=False)
